...
import { requireAdmin, AuthRequest, SESSION_COOKIE_NAME } from '../middleware/auth';
...
router.get('/', validateQuery(PaginationQuerySchema.extend({ include_hidden: z.coerce.boolean().optional() })), async (req: AuthRequest, res, next) => {
  try {
