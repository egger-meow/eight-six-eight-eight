...
import { createBookingNotificationEvent, kickNotificationWorker } from '../lib/notifications';
import { validateBookingMutation } from '../lib/booking-rules';
...
router.post('/', validate(BookingCreateSchema), async (req, res, next) => {
  try {
    ...
    let notificationEventId: number | null = null;
    const booking = await db.$transaction(async (tx) => {
      ...
      notificationEventId = await createBookingNotificationEvent({
        tx,
        booking: created,
    ...
    if (notificationEventId) kickNotificationWorker();
...
router.put('/:id', requireAdmin, doubleCsrfProtection, validateParams(IdParamSchema), validate(BookingUpdateSchema), async (req, res, next) => {
  try {
    ...
    if (updateData.roomId || updateData.checkIn || updateData.checkOut || updateData.guestCount) {
      ...
      const validation = await validateBookingMutation({ bookingId: id, roomId, checkIn, checkOut, guestCount });
      if (validation.error) {
        return res.status(validation.error.status).json({ success: false, error: { code: validation.error.code, message: validation.error.message } });
      }
    }
...
router.delete('/:id', requireAdmin, doubleCsrfProtection, validateParams(IdParamSchema), async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    const existingBooking = await db.booking.findUnique({ where: { id }, include: { room: true } });
    if (!existingBooking) {
      return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: '預約不存在' } });
    }
