...
function mapHolidayPeriodToResponse(period: any) {
  return {
    ...
    pricing_type: period.pricingType || inferPricingTypeFromName(period.name),
...
function setupRequiredMessage(errorCode?: string) {
  return errorCode === 'P2022'
    ? '特殊日期資料表需要更新，請先套用最新資料庫 schema。'
    : '特殊日期資料表尚未建立，請先套用資料庫 schema。';
}

async function findLegacyHolidayPeriods() {
  const rows = await db.$queryRawUnsafe<LegacyHolidayPeriodRow[]>(
    'SELECT id, name, "startDate", "endDate", "createdAt", "updatedAt" FROM "HolidayPeriod" ORDER BY "startDate" ASC, id ASC',
  );
  return rows.map(mapHolidayPeriodToResponse);
}
...
router.get('/', requireAdmin, async (req, res, next) => {
  try {
    ...
    if (error?.code === 'P2022') {
      const data = await findLegacyHolidayPeriods();
      return res.json({ success: true, data, meta: { pricing_type_setup_required: true } });
    }
    if (error?.code === 'P2021') {
...
router.post('/', requireAdmin, doubleCsrfProtection, validate(HolidayPeriodCreateSchema), async (req, res, next) => {
  try {
    ...
    const period = await db.holidayPeriod.create({ data: { name: req.body.name, startDate, endDate, pricingType: req.body.pricing_type || 'weekend' } });
    res.status(201).json({ success: true, data: mapHolidayPeriodToResponse(period) });
  } catch (error: any) {
    if (['P2021', 'P2022'].includes(error?.code)) {
      return res.status(409).json({ success: false, error: { code: 'SETUP_REQUIRED', message: setupRequiredMessage(error?.code) } });
    }
    next(error);
  }
});
...
router.put('/:id', requireAdmin, doubleCsrfProtection, validateParams(IdParamSchema), validate(HolidayPeriodUpdateSchema), async (req, res, next) => {
  try {
    ...
    if (!existing) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: '特殊日期區間不存在' } });
    ...
    const period = await db.holidayPeriod.update({
      ...
      data: {
        ...
        ...(req.body.pricing_type !== undefined ? { pricingType: req.body.pricing_type } : {}),
      },
    });
...
router.delete('/:id', requireAdmin, doubleCsrfProtection, validateParams(IdParamSchema), async (req, res, next) => {
  try {
    ...
    res.json({ success: true, data: { message: '特殊日期區間已刪除' } });
  } catch (error: any) {
    if (['P2021', 'P2022'].includes(error?.code)) {
      return res.status(409).json({ success: false, error: { code: 'SETUP_REQUIRED', message: setupRequiredMessage(error?.code) } });
    }
