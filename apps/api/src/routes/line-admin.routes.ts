...
router.post('/admin/binding-codes', requireAdmin, doubleCsrfProtection, async (req, res, next) => {
  try {
    const role = req.body?.role === 'owner' ? 'owner' : 'developer';
    const code = generateBindingCode(role);
    const expiresAt = new Date(Date.now() + config.LINE_BINDING_CODE_TTL_MINUTES * 60 * 1000);
    const binding = await db.lineBindingCode.create({
      data: {
...
router.get('/admin/admins', requireAdmin, async (_req, res, next) => {
  try {
    const admins = await db.lineAdmin.findMany({ orderBy: [{ role: 'asc' }, { boundAt: 'asc' }] });
    res.json({
      success: true,
      data: admins.map((admin) => ({
        ...
        role: admin.role,
        active: admin.active,
        valid_line_user_id: isLineUserId(admin.lineUserId),
        notification_eligible: admin.active && isLineUserId(admin.lineUserId),
...
router.get('/admin/notification-deliveries', requireAdmin, async (req, res, next) => {
  try {
...
router.put('/admin/notification-deliveries/:id/retry', requireAdmin, doubleCsrfProtection, async (req, res, next) => {
  try {
    ...
    const existing = await db.notificationDelivery.findUnique({ where: { id } });
    if (!existing) {
...
router.get('/admin/audit-logs', requireAdmin, async (_req, res, next) => {
  try {
    const logs = await db.lineAuditLog.findMany({
...
router.put('/admin/admins/:id/revoke', requireAdmin, doubleCsrfProtection, async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    const admin = await db.lineAdmin.findUnique({ where: { id } });
    if (!admin) {
...
function validLineSignature(rawBody: Buffer, signature: string | undefined) {
  if (!config.LINE_CHANNEL_SECRET || !signature) return false;
...
async function handleLineEvent(event: LineWebhookEvent) {
  if (event.webhookEventId) {
    try {
      await db.lineWebhookRedelivery.create({
        data: {
  ...
  const admin = await db.lineAdmin.findUnique({ where: { lineUserId } });
  ...
  await db.lineAdmin.update({ where: { id: admin.id }, data: { lastSeenAt: new Date() } });
...
async function tryBindLineAdmin(lineUserId: string, text: string) {
  const trimmed = text.trim();
  if (!trimmed.startsWith('8688-')) return false;

  const binding = await db.lineBindingCode.findUnique({ where: { codeHash: hashBindingCode(trimmed) } });
  if (!binding || !binding.active || binding.usedAt || binding.expiresAt <= new Date()) return false;
...
async function buildDashboardText() {
  ...
  const [checkIns, checkOuts, occupied, totalRooms, pending, upcoming] = await Promise.all([
    db.booking.findMany({ where: { checkIn: today, status: { notIn: ['cancelled', 'no_show'] } }, include: { room: true }, orderBy: { checkIn: 'asc' } }),
    db.booking.findMany({ where: { checkOut: today, status: { notIn: ['cancelled', 'no_show'] } }, include: { room: true }, orderBy: { checkOut: 'asc' } }),
    db.booking.count({ where: { checkIn: { lte: today }, checkOut: { gt: today }, status: { in: ['confirmed', 'checked_in'] } } }),
    db.room.count(),
    db.booking.findMany({ where: { status: 'pending' }, include: { room: true }, orderBy: { checkIn: 'asc' }, take: 5 }),
    db.booking.findMany({ where: { checkIn: { gte: today, lte: sevenDaysLater }, status: { notIn: ['cancelled', 'no_show'] } }, include: { room: true }, orderBy: { checkIn: 'asc' }, take: 8 }),
  ]);
...
function formatCompactBookings(bookings: any[]) {
  if (bookings.length === 0) return '';
  return '\n' + bookings.map((booking) => `#${booking.id} ${dateOnly(booking.checkIn)} ${booking.guestName} ${roomLabel(booking.room)}`).join('\n');
}

async function replyBookingSearch(replyToken: string | undefined, query: string) {
  const numericId = Number(query);
  const bookings = Number.isInteger(numericId) && numericId > 0
    ? await db.booking.findMany({ where: { id: numericId }, include: { room: true, internalNotes: { orderBy: { createdAt: 'desc' }, take: 3 } } })
...
async function validateBookingStateTransition(booking: any, status: string) {
  ...
  if (status === 'checked_in' && (today < booking.checkIn || today >= booking.checkOut)) return '今日不在此訂單入住區間內，不能辦理入住。';
  if (status === 'checked_out' && booking.status !== 'checked_in') return '只有已入住訂單可以辦理退房。';
  if (status === 'no_show' && !['pending', 'confirmed'].includes(booking.status)) return '只有待確認或已確認訂單可標記未入住。';
  if (status === 'no_show' && today <= booking.checkIn) return '入住日尚未過，不能標記未入住。';
  return null;
}

async function mutateBookingStatusFromLine(replyToken: string | undefined, bookingId: number, status: string, actorLineAdminId: number) {
  const existing = await db.booking.findUnique({ where: { id: bookingId }, include: { room: true } });
  if (!existing) {
  ...
  const eventId = await db.$transaction(async (tx) => {
    ...
    return createBookingNotificationEvent({ tx, booking, eventType, dedupeKey: `${eventType}:${bookingId}:line:${Date.now()}`, source: 'line', actorLineAdminId });
  });
  if (eventId) kickNotificationWorker();
  await replyText(replyToken, `訂單 #${bookingId} 已更新為 ${status}。`);
}

async function addInternalNoteFromLine(replyToken: string | undefined, bookingId: number, content: string, actorLineAdminId: number) {
...
async function findRoomByRef(roomRef: string, tx: Prisma.TransactionClient | typeof db = db) {
  return findSharedRoomByRef(roomRef, tx);
}

async function validateBookingSchedule(args: { tx?: Prisma.TransactionClient; bookingId?: number; roomId: number; checkIn: Date; checkOut: Date; guestCount: number }) {
  const validation = await validateBookingMutation({
    client: args.tx,
...
async function updateBookingFieldFromLine(replyToken: string | undefined, bookingId: number, field: string, value: string, actorLineAdminId: number) {
  ...
  } else if (['入住', 'checkin'].includes(field)) updateData.checkIn = new Date(value);
  else if (['退房', 'checkout'].includes(field)) updateData.checkOut = new Date(value);
  else if (['人數', 'guests'].includes(field)) updateData.guestCount = Number(value);
  else if (['金額', 'price'].includes(field)) updateData.totalPrice = Number(value);
  else if (['電話', 'phone'].includes(field)) updateData.guestPhone = value;
  else if (['狀態', 'status'].includes(field)) updateData.status = value;
  else { await replyText(replyToken, '不支援的欄位。'); return; }
  ...
  if (updateData.roomId || updateData.checkIn || updateData.checkOut || updateData.guestCount) {
    ...
    if (invalid) { await replyText(replyToken, invalid); return; }
  }

  const eventId = await db.$transaction(async (tx) => {
    const fresh = await tx.booking.findUnique({ where: { id: bookingId } });
    if (!fresh) throw new Error('Booking disappeared during LINE update');
    if (updateData.roomId || updateData.checkIn || updateData.checkOut || updateData.guestCount) {
    ...
    return createBookingNotificationEvent({ tx, booking, eventType: updateData.status === 'confirmed' ? 'booking.confirmed' : updateData.status === 'cancelled' ? 'booking.cancelled' : 'booking.modified', dedupeKey: `booking.modified:${bookingId}:line:${Date.now()}`, source: 'line', actorLineAdminId });
  }).catch(async (error) => {
...
async function buildBlockedDatesText() {
  ...
  if (blocks.length === 0) return '目前沒有未來封鎖日期。';
...
async function createBlockedDateFromLine(replyToken: string | undefined, roomRef: string, start: string, end: string, reason: string, actorLineAdminId: number) {
...
async function replyConfirmRemoveBlock(replyToken: string | undefined, blockId: number) {
...
async function updateRoomAvailabilityFromLine(replyToken: string | undefined, roomRef: string, available: boolean, actorLineAdminId: number) {
  const room = await findRoomByRef(roomRef);
  if (!room) { await replyText(replyToken, '房型不存在。'); return; }
  await db.$transaction(async (tx) => {
...
async function getAnnouncement() {
  return db.news.findFirst({ orderBy: [{ pinned: 'desc' }, { publishedAt: 'desc' }, { createdAt: 'desc' }] });
}

async function buildAnnouncementText() {
  const announcement = await getAnnouncement();
  if (!announcement) return '目前沒有網站公告。';
  return `目前公告：\n${announcement.title}\n${announcement.content}\n顯示：${announcement.visible ? '是' : '否'}`;
}

async function updateAnnouncementFromLine(replyToken: string | undefined, title: string, content: string, actorLineAdminId: number) {
  const announcement = await getAnnouncement();
  await db.$transaction(async (tx) => {
...
async function handlePostback(event: LineWebhookEvent, actorLineAdminId: number) {
  ...
  if (action === 'confirm_booking' || action === 'cancel_booking' || action === 'check_in' || action === 'check_out' || action === 'no_show') {
    ...
    const eventId = await db.$transaction(async (tx) => {
      ...
      return createBookingNotificationEvent({
        tx,
        booking,
        eventType,
  ...
  if (action === 'remove_block') {
    ...
    const block = await db.blockedDate.findUnique({ where: { id: blockId } });
    if (!block) {
...
function hashBindingCode(code: string) {
