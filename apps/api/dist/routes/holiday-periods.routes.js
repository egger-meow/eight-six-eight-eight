...
function setupRequiredMessage(errorCode) {
    return errorCode === 'P2022'
        ? '特殊日期資料表需要更新，請先套用最新資料庫 schema。'
        : '特殊日期資料表尚未建立，請先套用資料庫 schema。';
}
async function findLegacyHolidayPeriods() {
    const rows = await db_1.db.$queryRawUnsafe('SELECT id, name, "startDate", "endDate", "createdAt", "updatedAt" FROM "HolidayPeriod" ORDER BY "startDate" ASC, id ASC');
...
router.post('/', auth_1.requireAdmin, csrf_1.doubleCsrfProtection, (0, validate_1.validate)(holiday_periods_schema_1.HolidayPeriodCreateSchema), async (req, res, next) => {
    try {
        ...
        const period = await db_1.db.holidayPeriod.create({ data: { name: req.body.name, startDate, endDate, pricingType: req.body.pricing_type || 'weekend' } });
...
router.put('/:id', auth_1.requireAdmin, csrf_1.doubleCsrfProtection, (0, validate_1.validateParams)(common_schema_1.IdParamSchema), (0, validate_1.validate)(holiday_periods_schema_1.HolidayPeriodUpdateSchema), async (req, res, next) => {
    try {
        ...
        if (!existing)
            return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: '特殊日期區間不存在' } });
