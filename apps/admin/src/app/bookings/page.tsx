...
export default function BookingsPage() {
  ...
  useEffect(() => {
    const fetchBooking = async () => {
      ...
      try {
        const res = await apiFetch(`/bookings/${selectedBookingId}`);
        const b = res?.data;
        if (!b) return;
  ...
  const handleBookingClick = (id: number) => setSelectedBookingId(id);
    ...
    <div className={styles.pageContainer}>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div className="card" style={{ width: '760px', maxWidth: '100%', maxHeight: '92vh', overflowY: 'auto' }}>
            <h2 style={{ marginBottom: '1.5rem' }}>{selectedBooking ? `編輯訂單 #${selectedBooking.id}` : '新增訂單'}</h2>
            <form onSubmit={handleSubmit}>
              ...
              {selectedBooking && (
                <>
                  <label className="form-group">
                    <span className="form-label">新增內部備註紀錄</span>
                    <textarea className="input-field" rows={3} value={detailNote} onChange={(e) => setDetailNote(e.target.value)} placeholder="例如：客人來電確認晚到" />
                  </label>
                  {selectedBooking.internal_notes?.length > 0 && (
                    <div style={{ marginTop: '1rem', borderTop: '1px solid var(--border-color)', paddingTop: '1rem' }}>
                      <strong>備註紀錄</strong>
                      {selectedBooking.internal_notes.map((note: any) => (
                        <div key={note.id} style={{ marginTop: '0.75rem', color: 'var(--text-secondary)' }}>
                          {note.content}<br />
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', marginTop: '2rem', flexWrap: 'wrap' }}>
                <div>
                  {selectedBooking && <button type="button" className="btn btn-secondary" style={{ color: 'var(--status-red)', borderColor: 'var(--status-red)' }} onClick={handleDelete}>刪除訂單</button>}
                </div>
