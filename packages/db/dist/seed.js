...
async function main() {
    ...
    if (fs.existsSync(roomsJsonPath)) {
        ...
        for (const roomData of roomsData) {
            ...
            console.log(`✅ Room ensured: ${room.nameZh}`);
            // Optional: seed media for the room
            if (roomData.images && Array.isArray(roomData.images)) {
                const target = `room_${room.slug}`;
                let imgSortOrder = 0;
                for (const imgUrl of roomData.images) {
                    imgSortOrder += 10;
                    // Simple check to avoid duplicates if URL matches
                    ...
                    if (!existingMedia) {
                        await db.media.create({
                            data: {
                                ...
                                url: imgUrl,
                                mimeType: 'image/jpeg', // Assumption for seed
                                sizeBytes: 0,
    ...
    // 3. Optional: seed pages from content.ts (we will just create placeholders)
    const pages = [
